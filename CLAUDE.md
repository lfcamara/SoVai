...
- `hooks/` — two hooks: the SessionStart bootstrap and a pre-edit reminder, plus the shared config resolver and the wiring that registers them. They remind; they do not enforce (ADR-0012, amended). Paths there are plugin-relative via `${CLAUDE_PLUGIN_ROOT}`, and path classification comes from the edited project's own config, never from this plugin.
