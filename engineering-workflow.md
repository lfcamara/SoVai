...
Four hooks now do. What a given file *is* — production logic, UI, or test — is never guessed by the plugin: it comes from the project's own config, described under [Per-project configuration](#per-project-configuration), and a project without that file is not gated at all.
