...
## Tracker selection moved to sovai.config.json, and Linear stopped being the default

**Status: accepted, 2026-08-31.** Amends the paragraph above that routed tracker selection through `docs/agents/issue-tracker.md` with Linear as the fallback.

Three files were being asked to hold one class of fact. `docs/agents/issue-tracker.md` named the tracker, the target repo's `CLAUDE.md` held the Linear team and project, and `sovai.config.json` — already at the project root, already resolved by walking up, already the per-project home for everything else the plugin needs to know — held neither. A fact with three possible homes is a fact that gets written in the fourth place and found in none.

Tracker selection now lives in `sovai.config.json` under a `tracker` key: `kind`, whatever fields that tracker needs to identify a destination, and whether phases map to parent issues. `docs/agents/issue-tracker.md` survives for what it was actually good at — a house convention in prose that no schema would capture — and refines the config rather than competing with it. The key is read by skills rather than by the hooks, which is the one asymmetry in that file and is called out in its own comment.

Linear as the default is withdrawn. The paragraph above already conceded it was "the least-verified path rather than the safe one"; keeping it as the fallback meant an unconfigured project either published into somebody's wrong project or failed against a service that was never connected, and both of those read to a new user as the plugin being broken rather than as a missing line of config. The floor takes its place: with no `tracker` key, `to-tickets` asks once and offers to write the answer, and publishes to local markdown when nobody is there to ask. Falling to the floor loudly is a working outcome. A wrong guess is not.

Per-tracker mechanics moved out of the skill body into `TRACKERS.md` beside it. That is a genuine branch — a run reaches exactly one tracker's section — and so it is the one piece of this change that progressive disclosure actually justifies. The skill now states publishing in terms of four capabilities every tracker has some name for: create an issue, express a blocking edge, set a parent, set state. Adding a tracker is a section in that file, never a branch in the skill.
