...
Where an effort is in play, resolve it and read the phase's entry in `docs/planning/<effort>/<effort> — Roadmap.md` for what this phase ships, plus the effort's Wireframes and that phase's spec for the shape already agreed. This skill runs per phase, often in a fresh session weeks after the roadmap was drawn, so the plan is on disk rather than in the conversation. A prototype exploring a question the wireframes already settled is a prototype spent on nothing.
