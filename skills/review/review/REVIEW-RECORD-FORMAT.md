...
- **Every finding carries a cause.** It is the only part of the record that cannot be reconstructed from the diff afterward, and it is what tells a recurring process hole from a one-off defect when someone reads a run of these back. Name the gap ("the spec never said which seam"), not the moment ("missed it").
