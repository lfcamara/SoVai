...
- Planning — brainstorm (shape a raw idea, one question at a time) → to-prd → to-spec → to-wireframes → to-roadmap → to-tickets. Each stage hands to the next in the same conversation; continuing needs no named invocation. to-wireframes is the one exception: it stops at a design brief the user takes to a design tool, and resumes when they come back with a canvas. `grilling` is the interview underneath it.
- Prototyping — prototype (throwaway code answering one design question). Offered when a spec's Risks and unknowns names something only running code settles; never run automatically.
